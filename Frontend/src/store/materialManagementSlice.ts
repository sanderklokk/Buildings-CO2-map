 ...
 */
export const createMaterialManagementSlice: StateCreator<
  MaterialManagementSlice
> = (set) => ({
  materialManagementSlice: {
    materials: [],
    ...
    replaceMaterials: (materials: APIMaterialType[]) => {
      set((state) => ({
        materialManagementSlice: {
          ...state.materialManagementSlice,
          materials: [
            ...state.materialManagementSlice.materials.filter(
              (mat) => !materials.map((m) => m.id).includes(mat.id),
            ),
            ...materials,
          ],
        },
    ...
    removeMaterial: (id: number) => {
      set((state) => ({
        materialManagementSlice: {
          ...state.materialManagementSlice,
          materials: state.materialManagementSlice.materials.filter(
            (mat) => mat.id !== id,
          ),
        },
      }));
