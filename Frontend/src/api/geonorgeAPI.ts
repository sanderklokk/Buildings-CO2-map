...
export const GET_COORDINATES_FROM_ADDRESS = "https://ws.geonorge.no/adresser/v1/sok";
...
export const get_address_search = async (params: AdresseSokParameters): Promise<AxiosResponse<APIAdresseSok>>  => {
    if (!params.query && !params.gardsnummer && !params.bruksnummer && !params.festenummer) {
        throw new Error("error: tomt søk");
    }

    return await service.get(GET_COORDINATES_FROM_ADDRESS, {params: {
        sok: params.query,
