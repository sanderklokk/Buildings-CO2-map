...
export const MapBuildingPopup = ({
  ...
  const {
    ...
    queryKey: ["buildingdata", building.building],
    queryFn: () => get_detailed_building(building.building),
  });
  const {
    data: addressdata,
    ...
    queryKey: ["addressdata", coordinates],
    queryFn: () =>
      get_address_from_coordinates(coordinates.long, coordinates.lat),
  });
  ...
  return (
    <Box flex={1} display="flex" flexDirection="column" gap={0}>
      ...
      {addressdata &&
        (addressdata.data.metadata.totaltAntallTreff > 0 ? (
