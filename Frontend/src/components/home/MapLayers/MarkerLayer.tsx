...
export const MarkerLayer = () => {
  ...
  return (
    <>
      {hurtigSokResult && (
        <Marker
          position={[hurtigSokResult.longitude, hurtigSokResult.latitude]}
        >
          <Popup>
            <MapBuildingPopup
              building={hurtigSokResult}
              coordinates={{
                lat: hurtigSokResult.latitude,
                long: hurtigSokResult.longitude,
              }}
            />
          </Popup>
        </Marker>
      ...
      {zoom > 17 &&
        buildings
          ...
          .map((p, i) => {
            return (
              <Marker key={i} position={[p.longitude, p.latitude]}>
                <Popup>
                  <MapBuildingPopup
                    building={p}
