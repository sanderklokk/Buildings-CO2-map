...
export const BuildingForm = () => {
    ...
    return <Box marginBottom={2}>
        ...
        <Box display={"flex"} gap={3} alignItems={"start"} >
            <TextField data-testid="reportform-search-inp" required name="addressinput" label="Adresse" aria-label="Adresse søk input" type="text" className="w-[200px] mx-auto" placeholder="Adresse" variant="filled" value={addressInput} onChange={handleSearchChange} />
            <Button data-testid="reportform-search-btn" variant="contained" color="primary" size="medium" onClick={handleSearch} className="self-center">Søk</Button>
        </Box>
        {
            performSearch && (
                <Box marginTop={2} display={"flex"} flexDirection={"column"} gap={2}>
                    ...
                    {addressResults && <Box>
                        {addressResults.data && addressResults.data.adresser.map((v, i) =>
                            <Box flex={1} my={1} key={i} flexDirection={"row"}><Button className="" size="small" variant="outlined" onClick={() => handleSelectAddress(v)} data-testid={"select-address-"+v.adressetekst+"-btn"}>Velg</Button><Typography className="inline" ml={2}>{v.adressetekst}</Typography></Box>
                        )}
        ...
        { selectedAddress != null && selectedBuilding != null && <Box gap={2} flex={1} mt={2} flexDirection={"row"}>
            ...
            <Button variant="outlined" size="small" onClick={handleRemoveAddress} data-testid="reportform-remove-adr-btn"  className="self-center">Fjern</Button>
            </Box>}
        {errorMsg && <Typography mt={2} color={"red"}>{errorMsg}</Typography>}
