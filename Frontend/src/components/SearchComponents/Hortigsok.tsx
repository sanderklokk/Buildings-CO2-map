...
const Hortigsok = () => {
  ...
  const [searchResult, setSearchResult] =
    useState<AxiosResponse<APIPunktSok> | null>();
  ...
  return (
    <Box className="p-4">
      <TextField
        fullWidth
        label="Hurtigsøk på bygg"
        variant="outlined"
        value={formInputs.query}
        onChange={(e) =>
          setFormInputs({ ...formInputs, query: e.target.value })
        }
      />
      ...
      {showDetailed && (
        <Box className="mt-2 flex justify-center items-start space-x-2">
          <TextField
            ...
            label="Gårdsnr"
            variant="outlined"
            value={formInputs.gardsnummer}
            onChange={(e) =>
              setFormInputs({ ...formInputs, gardsnummer: e.target.value })
            }
          />
          <TextField
            className="w-1/3"
            label="Bruksnr"
            variant="outlined"
            value={formInputs.bruksnummer}
            onChange={(e) =>
              setFormInputs({ ...formInputs, bruksnummer: e.target.value })
            }
          />
          <TextField
            className="w-1/3"
            label="Seksjonsnr"
            variant="outlined"
            value={formInputs.festenummer}
            onChange={(e) =>
              setFormInputs({ ...formInputs, festenummer: e.target.value })
            }
          />
        </Box>
      )}
        ...
        <>
          {searchResult?.data && (
            <Typography className="">
              Resultater (
              {searchResult?.data.metadata.totaltAntallTreff > 10
                ? 10
                : searchResult?.data.metadata.totaltAntallTreff}
              )
            </Typography>
          )}
            ...
            >
              {searchResult?.data.adresser.map((address, i) => (
                <Box key={i} borderBottom={"1px solid #ccc"} padding={1}>
                  <Typography>{address.adressetekst}</Typography>
                  <Typography>{address.kommunenavn}</Typography>
                  <Button
                    data-testid={"hurtigsok-select-adress-" + i}
                    ...
                    onClick={() =>
                      handleSelectResult({
                        ...
                        long: address.representasjonspunkt.lon,
                      })
                    }
                  >
                    Velg
          ...
          {searchResult?.status != 200 && (
            <>
              <Typography className="">Resultater (0)</Typography>
              <Typography className="">Det oppsto en feil.</Typography>
            </>
          )}
          {loadingSearch && (
            <>
              <Typography className="">Resultater</Typography>
              <Typography className="text-sm">Laster resultater...</Typography>
            </>
          )}
        </>
      )}
