...
describe("BuildingForm", () => {
  ...
  it("check search called correctly", async () => {
    render(<BuildingForm />);
    const searchinp = within(
      screen.getByTestId("reportform-search-inp"),
    ).getByRole("textbox");
  ...
  it("check search results rendered", async () => {
    ...
    expect(
      screen.getByTestId(
        "select-address-" +
          mockAddressSearchData.adresser[0].adressetekst +
          "-btn",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId(
        "select-address-" +
          mockAddressSearchData.adresser[1].adressetekst +
          "-btn",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("check select and remove address", async () => {
    ...
    const selectaddressbtn = screen.getByTestId(
      "select-address-" +
        mockAddressSearchData.adresser[0].adressetekst +
        "-btn",
    );
    ...
    expect(
      Store.useBoundStore().wasteReportForm.setBygning,
    ).toHaveBeenCalledWith(mockClosestbuilding.byggningsnr);
  ...
  it("check cant find building error", async () => {
    ...
    expect(
      screen.getByText("Det oppstod en feil / Bygning ikke registrert."),
    ).toBeInTheDocument();
  });
  ...
  it("check search error", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Feil ved henting av adresser"),
      ).toBeInTheDocument();
    });
  });
});
