...
describe("AvfallsMaterialeRow", () => {
  ...
  it("check render the row and inputs", () => {
    ...
    expect(
      screen.getByTestId("form-avfallsrow-plannedamount"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("form-avfallsrow-actualamount"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("form-avfallsrow-difference"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("form-avfallsrow-tofacility"),
    ).toBeInTheDocument();
  ...
  it("check correct updating of row state", async () => {
    ...
    await userEvent.type(
      within(screen.getByTestId("form-avfallsrow-plannedamount")).getByRole(
        "spinbutton",
      ),
      planned,
    );
    await userEvent.type(
      within(screen.getByTestId("form-avfallsrow-actualamount")).getByRole(
        "spinbutton",
      ),
      actual,
    );
    await userEvent.type(
      within(screen.getByTestId("form-avfallsrow-tofacility")).getByRole(
        "spinbutton",
      ),
      tofactility,
    );
    await userEvent.type(
      within(screen.getByTestId("form-avfallsrow-toreuse")).getByRole(
        "spinbutton",
      ),
      reuse,
    );
    await userEvent.type(
      within(screen.getByTestId("form-avfallsrow-facility")).getByRole(
        "textbox",
      ),
      facility,
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("form-avfallsrow-difference"),
      ).toHaveTextContent(difference.toString());

      const { wasteReportForm } = useBoundStore.getState();
      const row = wasteReportForm.wasteReport.avfall.ordinert.find(
        (i) => i.id === mockmaterial1.id,
      );
      if (!row) {
