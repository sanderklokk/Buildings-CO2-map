...
const render = (element: React.ReactElement) => {
    ...
    return normal_render(<QueryClientProvider client={queryClient}>{element}</QueryClientProvider>)
...
describe("AvfallsplanForm", () => {
    ...
    it("check add material", async () => {
        ...
        await waitFor(() => {
            ...
            const addedrow = wasteReportForm.wasteReport.avfall.ordinert.find((i) => i.id === 3);
    ...
    it("check add parent material then sub", async () => {      
        ...
        await waitFor(() => {
            const { wasteReportForm } = useBoundStore.getState();
            const addedrow = wasteReportForm.wasteReport.avfall.ordinert.find((i) => i.id === 1);
            if (addedrow == undefined) {
                throw new Error("row not added correctly");
            }
        });
    ...
    it("check no submaterials message", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("Ingen gjenværende underkategorier")).toBeInTheDocument();
        });
    });
    ...
    it("check recursive submaterials", async () => {
        ...
        await waitFor(() => {
            const { wasteReportForm } = useBoundStore.getState();
            const addedrow = wasteReportForm.wasteReport.avfall.ordinert.find((i) => i.id === 6);
            if (addedrow == undefined) {
                throw new Error("row not added correctly");
            }
        });
    ...
    it("check farlig material added", async () => {
        ...
        await waitFor(() => {
            const { wasteReportForm } = useBoundStore.getState();
            const addedrow = wasteReportForm.wasteReport.avfall.farlig.find((i) => i.id === 7);
            if (addedrow == undefined) {
                throw new Error("row not added correctly");
            }
        });
