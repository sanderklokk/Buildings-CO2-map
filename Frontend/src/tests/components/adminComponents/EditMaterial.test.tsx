...
describe('EditMaterial', () => {
  ...
  it('check correct render', () => {
    render(<EditMaterial material={mockmaterial} open={true} onClose={vi.fn()} onSave={vi.fn()} />);
  ...
  it('check add a subcategory', async () => {
    vi.mocked(create_materialtype).mockResolvedValueOnce({ status: 200, statusText: 'OK', data: mocksubmaterial, headers: {}, config: { headers: new AxiosHeaders() } });

    render(<EditMaterial material={mockmaterial} open={true} onClose={vi.fn()} onSave={vi.fn()} />);

    fireEvent.change(screen.getByLabelText('Ny undermateriale'), { target: { value: 'ny sandtype' } });
  ...
  it('check delete submaterial works', async () => {
    ...
    fireEvent.click(screen.getByTestId("delete-submaterial-button-" + mocksubmaterial.id));
    ...
    await waitFor(() => {
      ...
      expect(mockStore.materialManagementSlice.removeMaterial).toHaveBeenCalledWith(mocksubmaterial.id);
    });
  ...
  it('check create material', async () => {
    ...
    vi.mocked(create_materialtype).mockResolvedValueOnce({ status: 200, statusText: 'OK', data: createdmaterial, headers: {}, config: { headers: new AxiosHeaders() } });

    render(<EditMaterial material={mockmaterial} open={true} onClose={vi.fn()} onSave={vi.fn()} />);
    const selectparent = screen.getByLabelText('Overordnet kategori') as HTMLSelectElement;
    ...
    fireEvent.change(screen.getByLabelText('Ny undermateriale'), { target: { value: 'minigrus' } });
  ...
  it('check set material hidden and not hidden', async () => {
    const hiddenmaterials = [mockmaterial, mocksubmaterial, mocksubsubmaterial].map((mat) => ({ ...mat, synlig: false }));

    vi.mocked(update_materialtypes).mockResolvedValueOnce({ status: 200, statusText: 'OK', data: hiddenmaterials, headers: {}, config: { headers: new AxiosHeaders() } });
    ...
    await waitFor(() => {
      expect(mockStore.materialManagementSlice.replaceMaterials).toHaveBeenCalledWith(hiddenmaterials);
    });

    const nothiddenmaterials = [mockmaterial, mocksubmaterial, mocksubsubmaterial].map((mat) => ({ ...mat, synlig: true }));

    vi.mocked(update_materialtypes).mockResolvedValueOnce({ status: 200, statusText: 'OK', data: nothiddenmaterials, headers: {}, config: { headers: new AxiosHeaders() } });
    ...
    await waitFor(() => {
      expect(mockStore.materialManagementSlice.replaceMaterials).toHaveBeenCalledWith(nothiddenmaterials)
    });
