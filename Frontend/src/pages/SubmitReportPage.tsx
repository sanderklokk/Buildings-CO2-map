...
import {
  AvfallsMaterialeRow,
  WasteReport,
} from "../store/wastereportFormSlice";
...
export const SubmitReportPage = () => {
  ...
  const toApiMaterial = (
    material: AvfallsMaterialeRow,
  ): APIWasteReportMaterial => ({
    id: null,
  ...
  const handleSubmit = async () => {
    const wastematerials: APIWasteReportMaterial[] = wasteReport.avfall.farlig
      .concat(wasteReport.avfall.ordinert)
      .map(toApiMaterial);
    const data: APIWasteReport = {
      ...toApiReport(wasteReport),
      materialer: wastematerials,
    };
    ...
    try {
      ...
      if (isAxiosError(err) && err.response) {
        ...
        if (res.status === 400) {
          setSnackbarData({
            show: true,
            message: "Rapporten mangler felter",
            color: "#660000",
            severity: "error",
          });
